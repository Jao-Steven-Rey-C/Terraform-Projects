locals {
  ...
  logical    = true || false #Logical operators: OR ||, AND &&, NOT !
  ...
  fullnames = [for person in var.objects_list : "Engr. ${person.firstname} ${person.middlename} ${person.lastname}"]

  #Maps
  farenheit_map             = { for key, value in var.temperatures_celcius_map : key => (value * 9 / 5) + 32 } //'key' and 'value' are temporary variables.
  lucky_chinese_numbers_map = { for key, value in var.numbers_map : key => value/8 if(value - 8) % 10 == 0 }

  #Lists + Maps
  users_map = { for user_info in var.users : user_info.username => user_info.role... } //List to map

  users_map2 = {
    for username, roles in local.users_map : username => { task = roles } //Adds 'roles' on top of roles
  }
